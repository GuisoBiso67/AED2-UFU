...
/*
int duploHash(int H1, int chave, int i, int TABLE_SIZE){ // duplo hash original
    int H2 = chaveDivisao(chave,TABLE_SIZE-1) + 1;
    return ((H1 + i*H2) & 0x7FFFFFFF) % TABLE_SIZE;
}
*/

int duploHash(int H1, int chave, int i, int TABLE_SIZE){ // apenas numeros impares
    int H2 = chaveDivisao(chave, TABLE_SIZE-1) + 1;
    if ((H2 % 2) == 0) H2 = H2 + 1;
    return ((H1 + i*H2) & 0x7FFFFFFF) % TABLE_SIZE;
}
    ...
    for(i=0; i < ha->TABLE_SIZE; i++){
        ...
        if(ha->itens[newPos] == NULL){
            ...
            *novo = al;
            ...
            // Se i > 0, significa que houve colisões.
            // Somamos 'i' ao total acumulado de colisões.
            *colisoes = *colisoes + i;
            // ------------------------------
            return 1;
        }
